import { Client, Collection, DMChannel, GuildEmoji, GuildMember, Message, NewsChannel, OAuth2Guild, TextChannel, ThreadChannel, User, Webhook } from "discord.js";
...
//Signed, Pyro
const fetchWebhook = async (message: Message): Promise<Webhook> => {
...
const fixPoorMessage = async (message: Message, client: Client): Promise<string | undefined> => {

    let match = message.content.match(/:(.+?):/g)
    let guilds = await client.guilds.fetch();
    let guildEmojis, emojis: Collection<string, GuildEmoji>;
    emojis = new Collection([])
    for(const guild of guilds.values()) {
        let baseGuild = await guild.fetch();
        guildEmojis = await baseGuild.emojis.fetch();

        emojis = new Collection([...emojis, ...guildEmojis.filter(emoji => match!.includes(`:${emoji.name}:`))])
    }
...
const sendFixedMessage = async (message: string, author: GuildMember, webhook: Webhook, threadId: string | undefined) => {
    webhook.send({
        content: message,
