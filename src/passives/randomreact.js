...
module.exports = {
  ...
  async execute(message) {
    const [react_channels] = await configdb.models.React_Channels.findOrCreate({
      where: { ConfigServer: message.guildId },
    });

    //don't do random reactions if the mode is allow and the channel isn't in the list
    //otherwise don't do random react if the mode is disallow and the channel is in the list
    const reactChannels = react_channels.channels.includes(parseInt(message.channelId));
    if ((react_channels.allow && !reactChannels) || (!react_channels.allow && reactChannels) ) { return; }
    ...
    const Reacts = async (message, client) => {
      ...
      if (rand <= chance) {
        ...
        AllEmotes = AllEmotes.filter(
          (e) => !AllEmotesfilter.includes(`${e.id}`)
        );
        ...
        if (EmojiGot == undefined) {
          return;
        }

        //And react with the emote.
        message.react(EmojiGot).catch(async (err) => {
    ...
    if (
      message.content.match(/(?!<a?|`.*):[^<:>\s]+?:(?!\d+>|.*`)/g) === null
    ) {
