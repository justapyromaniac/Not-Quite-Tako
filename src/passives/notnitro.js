module.exports = {
  ...
  async execute(message) {
    ...
    const fetchWebhook = async (message) => {
      ...
      let notQuiteTako = webhooks.find(
      ...
      if (notQuiteTako === undefined /*&& message.channel.type !== "DM"*/) {
        if (message.channel.isThread()) {
          notQuiteTako = await message.channel.parent.createWebhook({
            name: webhookName,
          });
        } else {
          notQuiteTako = await message.channel.createWebhook({
            name: webhookName,
          });
        }
      }
    ...
    if(CorrectEmojis){
     for(let Emoji of CorrectEmojis){
      ...
      let filteredEmojis = emojis.filter((emoji) =>
        match.includes(`:${emoji.name}:`)
      );
      filteredEmojis.sweep(
        (emoji, key) => guildEmojis.has(key) && !emoji.animated
      );
